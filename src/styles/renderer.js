...
const EL_COLORS = () => {
  ...
  return dark ? EL_COLORS_DARK : EL_COLORS_LIGHT;
};
...
function renderQuatroPilares(fourPillars, tenGods, container) {
  ...
  pilares.forEach(({ pilar, label, zh, isDia, tgIdx }) => {
    ...
    // ── Cores por elemento ───────────────────────────────────────────────────
    const stemColors   = EL_COLORS()[elKey(tronco.el)] || {};
    const branchColors = EL_COLORS()[elKey(ramo.el)]   || {};

    // ── Outline dourado fino no pilar do Dia ─────────────────────────────────
    if (isDia) {
      /* border dourado via .is-day-master — sem outline duplo */
    }

    // ── Tronco Celestial — DS v2.5 var C: fundo=dark(.bg), hanzi=light(.fg) ──
    const stemWrap = el('div', ['bazi-pillar__stem']);
    stemWrap.style.background   = stemColors.bg || '';
    stemWrap.style.borderBottom = 'none';
    // traço fino interno para unidade visual entre as metades
    stemWrap.style.boxShadow = 'inset 0 0 0 1px rgba(255,255,255,0.08)';
    ...
    // DS v2.5 variação C: dark no bg, light no hanzi
    stemChar.style.color = stemColors.fg || 'rgba(255,255,255,0.95)';
    stemWrap.appendChild(stemChar);

    // pinyin removido — sem etimologia, tradução já é direta para PT
    stemWrap.appendChild(elBadge(tronco.el));

    const stemYY = el('span', ['bazi-pillar__label']);
    stemYY.textContent = tronco.yin ? 'Yin' : 'Yang';
    stemYY.style.color   = 'rgba(255,255,255,0.72)';
    ...
    // ── Rótulo central: HORA / DIA / MÊS / ANO ──────────────────────────────
    const hd = el('div', ['bazi-pillar__header']);
    hd.textContent = `${label.toUpperCase()} · ${zh}${isDia ? ' ✦' : ''}`;
    card.appendChild(hd);

    // ── Ramo Terrestre — DS v2.5 var C: fundo=dark(.bg), hanzi=light(.fg) ────
    const branchWrap = el('div', ['bazi-pillar__branch']);
    branchWrap.style.background = branchColors.bg || '';
    branchWrap.style.boxShadow  = 'inset 0 0 0 1px rgba(255,255,255,0.08)';
    ...
    // DS v2.5 variação C: dark no bg, light no hanzi
    branchChar.style.color = branchColors.fg || 'rgba(255,255,255,0.95)';
    branchWrap.appendChild(branchChar);

    // pinyin do ramo removido — animal em PT já cobre a legenda
    ...
    branchSub.style.color = 'rgba(255,255,255,0.72)';
...
function renderForca(strength, favorable, dmStemIdx, container) {
  ...
  // 從格 — aviso de Estrutura de Rendição
  if (favorable?.congGe) {
    const row = el('div', ['str-row']);
    const lbl = el('span', ['str-lbl']); lbl.textContent = 'Estrutura (格局)';
    row.appendChild(lbl);
    const tag = el('span', ['str-verdict']);
    tag.textContent = '從格 Cóng Gé';
    row.appendChild(tag);
    card.appendChild(row);
  }

  // 用神 — Deus Útil principal (destaque)
  if (favorable?.yongShen) {
    const row = el('div', ['str-row']);
    const lbl = el('span', ['str-lbl']); lbl.textContent = 'Deus Útil — 用神 Yòng Shén';
    row.appendChild(lbl);
  ...
  // 喜神 — Auxiliares favoráveis (xiShen sem o yongShen)
  const xiDisplay = (favorable?.xiShen ?? []).filter(e => e && e !== favorable?.yongShen);
  if (xiDisplay.length) {
    const row = el('div', ['str-row']);
    const lbl = el('span', ['str-lbl']); lbl.textContent = 'Auxiliares — 喜神 Xǐ Shén';
    row.appendChild(lbl);
    const els = el('span', ['str-els']);
    xiDisplay.forEach(e => els.appendChild(elBadge(e)));
    row.appendChild(els);
    card.appendChild(row);
  }

  // 忌神 — Desfavoráveis
  if (favorable?.jiShen?.length) {
    const row = el('div', ['str-row']);
    const lbl = el('span', ['str-lbl']); lbl.textContent = 'Desfavoráveis — 忌神 Jì Shén';
    row.appendChild(lbl);
    const els = el('span', ['str-els']);
    favorable.jiShen.forEach(e => els.appendChild(elBadge(e)));
    row.appendChild(els);
    card.appendChild(row);
  }
...
function renderTermosSolares(solarTerms, birthYear, container) {
  ...
  wrap.appendChild(secLabel(`TERMOS SOLARES DE ${birthYear}`, '節氣'));
...
function renderEstrelasSimbolicas(stars, allBranches, container) {
  ...
  stars.forEach(star => {
    ...
    const card = el('div', ['star-card', found ? 'star-card--present' : '']);
...
function renderInteracoes(interactions, fourPillars, container) {
  ...
  wrap.appendChild(secLabel('INTERAÇÕES ENTRE RAMOS TERRESTRES', '地支'));
...
async function renderDezDeuses(mapa, dmStemIdx, container) {
  ...
  sec.setAttribute('aria-label', '十神 — 10 Deuses');

  // Header
  const header = el('div', ['dez-deuses-header']);
  sec.appendChild(header);
  header.appendChild(secLabel('10 DEUSES', '十神 · Shí Shén'));
