  ...
  var out = nameHtml + ziHtml
    ...
    + (function(){
        ...
        tds.forEach(function(td,i){
          ...
          var d=td.date;
          var dot='<span class="term-dot '+c+'" role="listitem">'+td.n+' '+p2(d.day)+'/'+p2(d.month)+'</span>';
          rows[i<6?0:1]+=dot;
        });
...
(function() {
  ...
  el.addEventListener('input', function() {
    ...
    if (digits.length <= 2) {
      ...
      var hh = clamp(parseInt(digits.slice(0,2),10),0,23);
      masked = String(hh).padStart(2,'0') + ':' + digits.slice(2);
    }
    if (digits.length === 4) {
      var hh2 = clamp(parseInt(digits.slice(0,2),10),0,23);
      var mm  = clamp(parseInt(digits.slice(2,4),10),0,59);
      this.value = String(hh2).padStart(2,'0')+':'+String(mm).padStart(2,'0');
      setValid(this,true); focusNext('inName'); updateRST(); return;
    }
