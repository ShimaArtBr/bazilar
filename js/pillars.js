...
function monthPil(jd) {
  // CORREÇÃO: encontrar o termo solar mais recente (maior JD <= jd)
  var y = fromJD(jd).year;
  var bestMi = -1;
  var bestJD = -Infinity;
  // Verificar anos y e y-1 (para cobrir casos de janeiro/fevereiro)
  for (var yr = y; yr >= y-1; yr--) {
    for (var i = 0; i < 12; i++) {
      var tjd = termJD(MT[i].l, yr);
  ...
  // fallback (nunca deve ocorrer, mas por segurança)
  if (bestMi === -1) bestMi = 0;
  var bi = MBS[bestMi];
  var ys = yearPil(jd).si;
  var si = (YMS[ys] + bestMi) % 10;
  return {si: si, bi: bi, mi: bestMi};
}

function dayPil(jd) {
  // CORREÇÃO: remover Math.round, usar jd diretamente (já é inteiro ao meio-dia)
  var idx = ((jd - 2451545 + 54) % 60 + 60) % 60;
